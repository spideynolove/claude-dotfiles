---
name: xia
description: Borrow and adapt patterns from GitHub projects into the current codebase using a structured 6-phase protocol. Invoke with /xia <github-repo> [focus].
argument-hint: "<github-repo> [focus]"
---
...
Extract and list every coding constraint found. These apply to ALL code written in this session.
...
**Never install directly to `~/.claude/`** unless the user explicitly says "install globally".
...
Score friction as: **Low** = drops in cleanly, **Med** = needs adaptation, **High** = conflicts with A's patterns.
...
**If `.claude/xia/XIALOGUE.md` does not exist**, create it:
...
**If it exists**, rewrite "Current evolved state of A" and append one row to the table.
