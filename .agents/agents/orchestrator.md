...
1. Read predecessor results if any:
   ```bash
   [ -f .aim/results/<predecessor_id>.json ] && cat .aim/results/<predecessor_id>.json
   ```

2. Spawn the task-runner subagent:
   ```
   @task-runner: Role: <role>. Tool: <tool>. Task ID: <tN>. Task: <description>. Context from predecessors: <predecessor output summaries>
   ```
